...
import type { CreditPackageRow } from '@/lib/admin-data/queries'
...
export function CreditPackageCreateForm() {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = (await createCreditPackage(formData)) as ActionResult
      if (result.success) {
        form.reset()
        setMessage(null)
        router.refresh()
      ...
      setMessage(result.message ?? 'No se pudo crear el paquete.')
    })
  }
  ...
  return (
    <form
      ...
      className="mb-6 grid gap-3 rounded-hero border border-slate/10 bg-white p-5 shadow-card md:grid-cols-[minmax(0,1fr)_120px_140px_110px_auto]"
    >
      <label className="space-y-1.5">
        <span className="text-xs font-bold uppercase tracking-eyebrow text-slate">Nuevo paquete</span>
        <input
          className="h-11 w-full rounded-xl border border-slate/15 px-3 text-sm outline-none focus:border-[#EE7070]"
          name="name"
          placeholder="Paquete 20"
          required
        />
      </label>
      <label className="space-y-1.5">
      ...
      {message && <p className="text-xs font-semibold text-red-600 md:col-span-5">{message}</p>}
    </form>
  )
}

export function CreditPackageActions({ pack }: { pack: CreditPackageRow }) {
  const router = useRouter()
  const [isPending, startTransition] = useTransition()
  ...
  function handleUpdate(event: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      refreshAfter((await updateCreditPackage(formData)) as ActionResult)
    })
  }

  function handleToggle() {
    startTransition(async () => {
      refreshAfter((await toggleCreditPackage(pack.id, !pack.is_active)) as ActionResult)
    })
  }

  if (isEditing) {
    return (
      <form onSubmit={handleUpdate} className="min-w-[32rem] space-y-2">
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        {isPending ? '...' : pack.is_active ? 'Desactivar' : 'Activar'}
      </button>
