...
export function PasswordRecoveryForm() {
  ...
  function handleReset(event: FormEvent<HTMLFormElement>) {
    ...
    if (
      ...
      toast.error('La contraseña debe tener 12+ caracteres, con al menos una letra y un número.')
      return
    }
      ...
      {step === 'reset' ? (
        <form onSubmit={handleReset} className="space-y-4 sm:space-y-5">
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#EE7070] px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-[#EE7070]/25 transition-all hover:bg-[#D94F4F] disabled:opacity-60 disabled:cursor-not-allowed active:scale-[0.98] motion-reduce:transition-none motion-reduce:active:scale-100"
          >
            {isPending ? <Loader2 className="h-4 w-4 animate-spin motion-reduce:animate-none" aria-hidden="true" /> : <Lock className="h-4 w-4" aria-hidden="true" />}
      ...
      {step === 'done' ? (
        <div className="rounded-2xl border border-green-100 bg-green-50 p-5 text-center sm:p-8">
          ...
          <div className="mt-5 flex flex-col gap-2.5 sm:mt-6 sm:flex-row sm:justify-center sm:gap-3">
            <Link href="/gestion-x7k2m9/iniciar-sesion" className="flex items-center justify-center gap-2 rounded-xl bg-[#EE7070] px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-[#EE7070]/25 transition-all hover:bg-[#D94F4F] active:scale-[0.98]">
...
function PasswordRequirements({ password, confirm }: { password: string; confirm: string }) {
  const checks = [
    { ok: password.length >= 12, label: 'Al menos 12 caracteres' },
    { ok: /[a-zA-Z]/.test(password), label: 'Incluye una letra' },
    { ok: /[0-9]/.test(password), label: 'Incluye un número' },
  ...
  return (
    <ul
      aria-live="polite"
      className="space-y-1.5 rounded-2xl border border-neutral-100 bg-neutral-50 px-3 py-2.5 sm:px-4 sm:py-3"
    >
      {checks.map((check) => (
        <li key={check.label} className="flex items-center gap-2 text-xs sm:text-sm">
          {check.ok ? (
            <CheckCircle2 className="h-4 w-4 flex-shrink-0 text-green-500" aria-hidden="true" />
          ) : (
            <Circle className="h-4 w-4 flex-shrink-0 text-neutral-300" aria-hidden="true" />
          )}
          <span className={check.ok ? 'font-medium text-neutral-700' : 'text-neutral-500'}>
            {check.label}
          </span>
